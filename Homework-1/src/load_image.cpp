...
void save_image_stb(const Image &im, const string &name, int png) {
  ...
  int success = 0;
  if (png)
    success = stbi_write_png(file.c_str(), im.w, im.h, im.c, data, im.w * im.c);
  else
    success = stbi_write_jpg(file.c_str(), im.w, im.h, im.c, data, 100);
...
void Image::save_png(const string &name) const {
  save_image_stb(*this, name, 1);
}

void Image::save_image(const string &name) const {
  save_image_stb(*this, name, 0);
}
...
void Image::load_image(const string &filename) {
  *this = load_image_stb(filename, 0);
}
